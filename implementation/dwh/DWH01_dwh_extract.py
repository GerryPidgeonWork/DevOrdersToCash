...
# Note: .parent.parent.parent because this file is in implementation/dwh/
...
from core.C02_set_file_paths import SQL_DIR, ensure_directory
from core.C07_datetime_utils import get_month_range, format_date
from core.C09_io_utils import save_dataframe
from core.C12_data_processing import filter_rows
from core.C14_snowflake_connector import run_sql_to_dataframe
...
def get_date_range_from_period(accounting_period: str) -> Tuple[str, str]:
    ...
    try:
        ...
        start_date = format_date(first_day, "%Y-%m-%d")
        end_date = format_date(last_day, "%Y-%m-%d")
...
def main() -> None:
    """
    Description:
        Self-test for DWH01_dwh_extract module.
    ...
    Notes:
        - Self-test requires active Snowflake connection and valid Google Drive path.
        - For actual testing, call run_dwh_extraction() with appropriate parameters.
    """
    logger.info("=" * 60)
    logger.info("DWH01_dwh_extract.py - Self Test")
    logger.info("=" * 60)
    logger.info("This module should be called via G10b controller or run_dwh_extraction().")
    logger.info("For testing, use the GUI or import and call run_dwh_extraction() directly.")
