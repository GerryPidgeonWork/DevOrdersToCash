...
# AUDIT: Added get_end_of_week to replace manual timedelta(days=6)
from core.C07_datetime_utils import parse_date, format_date, get_end_of_week, try_parse_date
from core.C09_io_utils import save_dataframe
from core.C10_pdf_utils import extract_pdf_text
...
def process_single_pdf(
    ...
    statement_start = try_parse_date(m_period.group(1) if m_period else None)
    statement_end = try_parse_date(m_period.group(2) if m_period else None)
    ...
    payment_date = try_parse_date(m_payment.group(1) if m_payment else None)
...
def main() -> None:
    """
    Description:
        Self-test for JE01_parse_pdfs module.
    ...
    Notes:
        ...
        - For actual testing, call run_je_pdf_parser() with appropriate parameters.
    """
    logger.info("=" * 60)
    logger.info("JE01_parse_pdfs.py - Self Test")
    logger.info("=" * 60)
    logger.info("This module should be called via G10b controller or run_je_pdf_parser().")
    logger.info("For testing, provide pdf_folder, output_folder, and statement dates.")
