...
# (No additional imports required for static data module)
...
def main() -> None:
    """
    Description:
        Self-test for I03_project_static_lists module.
    ...
    Notes:
        - Validates that all exported constants are defined and non-empty.
    """
    logger.info("📋 I03_project_static_lists self-test started.")

    # ------------------------------------------------------------------
    # VALIDATE EXPORTS
    # ------------------------------------------------------------------
    logger.info("-" * 60)
    logger.info("Validating exported constants...")
    logger.info("-" * 60)

    exports = {
        "FINAL_DF_ORDER": FINAL_DF_ORDER,
        "JET_COLUMN_RENAME_MAP": JET_COLUMN_RENAME_MAP,
        "JUSTEAT_DWH_COLUMN_RENAME_MAP": JUSTEAT_DWH_COLUMN_RENAME_MAP,
        "JUSTEAT_RECON_COLUMN_ORDER": JUSTEAT_RECON_COLUMN_ORDER,
        "DELIVEROO_RECON_COLUMN_ORDER": DELIVEROO_RECON_COLUMN_ORDER,
        "DR_COLUMN_RENAME_MAP": DR_COLUMN_RENAME_MAP,
        "DR_COLUMN_ORDER": DR_COLUMN_ORDER,
        "DR_ACCOUNTING_CATEGORY_MAP": DR_ACCOUNTING_CATEGORY_MAP,
        "DR_MFC_MAPPING_FILENAME": DR_MFC_MAPPING_FILENAME,
    }

    for name, value in exports.items():
        if isinstance(value, (list, dict)):
            logger.info("  ✓ %s: %d items", name.ljust(35), len(value))
        else:
            logger.info("  ✓ %s: %s", name.ljust(35), value)

    logger.info("-" * 60)
    logger.info("✅ I03_project_static_lists self-test complete.")
