...
# ----------------------------------------------------------------------------------------------------
__all__ = [
    "GOPUFF_FILENAME_PATTERN",
    "DELIVEROO_FILENAME_PATTERN",
    "rename_gopuff_files",
    "extract_section",
    "enrich_orders_df",
    "parse_deliveroo_csv",
...
def main() -> None:
    """
    Description:
        Self-test for DR001_parse_csvs module.
    ...
    Notes:
        - Tests CSV parsing on sample folders if they exist.
        - For actual use, call run_dr_csv_parser() with appropriate parameters.
    """
    logger.info("DR001_parse_csvs self-test started.")
    ...
    logger.info("=== Testing run_dr_csv_parser ===")
    logger.info("CSV folder: %s", csv_folder)
    logger.info("Output folder: %s", output_folder)
    logger.info("Date range: %s -> %s", test_start, test_end)
    logger.info("Folders exist: CSV=%s, Output=%s", dir_exists(csv_folder), dir_exists(output_folder))

    if dir_exists(csv_folder) and dir_exists(output_folder):
        result = run_dr_csv_parser(
        ...
        if result:
            ...
            logger.warning("=== FAILED: No output generated ===")
    else:
        logger.error("Folder(s) do not exist!")
