...
from core.C02_set_file_paths import path_exists_safely
...
from core.C07_datetime_utils import (
    ...
    format_date,         # Format date with custom format string
)
...
def get_je_statement_coverage(
    ...
    for pdf_path in statement_folder.glob("*JE Statement*.pdf"):
        ...
        week_end = get_end_of_week(week_start)
...
__all__ = [
    ...
    # Date range helpers
    "statement_overlaps_range",
    # Just Eat statement helpers
    "get_je_statement_coverage",
    "get_je_order_detail_coverage",
    "calculate_statement_period",
    "calculate_accrual_period",
    # MFC mapping functions (Deliveroo)
    "load_mfc_mapping",
    "save_mfc_mapping",
    "add_mfc_mapping",
    "delete_mfc_mapping",
...
def main() -> None:
    """
    Description:
        Self-test for I02_project_shared_functions module.
    ...
    Notes:
        - Tests path validation and date helper functions.
        - Does not test MFC mapping (requires file system setup).
    """
    logger.info("🔧 I02_project_shared_functions self-test started.")

    # ------------------------------------------------------------------
    # DATE HELPER TESTS
    # ------------------------------------------------------------------
    logger.info("-" * 60)
    logger.info("Date Helper Tests")
    logger.info("-" * 60)

    # Test statement_overlaps_range
    test_cases = [
        # (stmt_start, stmt_end, range_start, range_end, expected)
        (date(2025, 1, 6), date(2025, 1, 12), date(2025, 1, 1), date(2025, 1, 31), True),   # Overlaps
        (date(2025, 2, 1), date(2025, 2, 7), date(2025, 1, 1), date(2025, 1, 31), False),   # After
        (date(2024, 12, 1), date(2024, 12, 7), date(2025, 1, 1), date(2025, 1, 31), False), # Before
        (date(2025, 1, 25), date(2025, 2, 5), date(2025, 1, 1), date(2025, 1, 31), True),   # Partial overlap
    ]

    for stmt_start, stmt_end, range_start, range_end, expected in test_cases:
        result = statement_overlaps_range(stmt_start, stmt_end, range_start, range_end)
        status = "✓" if result == expected else "✗"
        logger.info("  %s overlaps(%s-%s, %s-%s) = %s (expected %s)",
                    status, stmt_start, stmt_end, range_start, range_end, result, expected)

    # ------------------------------------------------------------------
    # PROVIDER VALIDATION TESTS (requires initialised paths)
    # ------------------------------------------------------------------
    logger.info("-" * 60)
    logger.info("Provider Validation Tests")
    logger.info("-" * 60)

    # Test without initialisation (should return False)
    result = validate_provider_ready("deliveroo")
    logger.info("  validate_provider_ready('deliveroo') without init: %s (expected False)", result)

    logger.info("-" * 60)
    logger.info("✅ I02_project_shared_functions self-test complete.")
