...
# Note: .parent.parent.parent because this file is in implementation/braintree/
...
from core.C06_validation_utils import file_exists, dir_exists
from core.C07_datetime_utils import format_date, try_parse_date
...
def parse_braintree_datetime(datetime_str: str) -> date | None:
    ...
    Notes:
        - Extracts the date portion from datetime strings that may include
          time and timezone components.
        - Uses C07's try_parse_date for the actual date parsing.
    """
    # Handle null/empty using C07's try_parse_date pattern
    if datetime_str is None:
        return None
    if isinstance(datetime_str, float) and pd.isna(datetime_str):
        return None
    datetime_str = str(datetime_str).strip()
    if not datetime_str:
        return None

    # Extract date portion from MM/DD/YYYY (with optional time/timezone)
    match = re.match(r"(\d{1,2}/\d{1,2}/\d{4})", datetime_str)
    if match:
        return try_parse_date(match.group(1), fmt="%m/%d/%Y")

    # Extract date portion from YYYY-MM-DD (with optional time)
    match = re.match(r"(\d{4}-\d{2}-\d{2})", datetime_str)
    if match:
        return try_parse_date(match.group(1), fmt="%Y-%m-%d")
...
def get_braintree_date_range(csv_path: Path) -> tuple[date, date] | None:
    ...
    """
    try:
        # Read CSV using C09's read_csv_file
        df = read_csv_file(csv_path, encoding="utf-8-sig", low_memory=False)
...
def rename_braintree_files(
    ...
    for month_key in sorted(month_groups.keys()):
        ...
        for seq_num, (csv_path, min_date, max_date) in enumerate(files, start=1):
            ...
            if file_exists(new_path) and new_path != csv_path:
...
# ----------------------------------------------------------------------------------------------------
__all__ = [
    "BRAINTREE_DATETIME_COLUMN",
    "BRAINTREE_STANDARD_PATTERN",
    "parse_braintree_datetime",
    "get_braintree_date_range",
    "analyse_braintree_files",
    "rename_braintree_files",
]
...
def main() -> None:
    """
    Description:
        Self-test for BT01_parse_csvs module.
    ...
    Notes:
        - Tests file renaming on a sample folder if it exists.
        - For actual use, call rename_braintree_files() with appropriate parameters.
    """
    logger.info("=" * 60)
    logger.info("BT01_parse_csvs.py - Self Test")
    logger.info("=" * 60)

    # Test folder
    test_folder = Path(r"H:\Shared drives\Automation Projects\Accounting\Orders to Cash\01 Braintree\01 CSVs\01 To Process")

    if dir_exists(test_folder):
        logger.info("Folder exists: %s", test_folder)
        rename_braintree_files(test_folder, log_callback=lambda msg: logger.info(msg))
    else:
        logger.warning("Test folder does not exist: %s", test_folder)
