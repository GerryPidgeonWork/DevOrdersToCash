...
**Important (Model Memory):** Even models with persistent memory or long-term context features MUST treat each session as a **cold start** for compliance purposes.
...
### Mandatory Read Receipt (Required)

After completing the mandatory reading for the project type, you MUST explicitly confirm completion before any implementation.

Your confirmation MUST include:

1. **Documents read** — list each file by name
2. **Key constraints retained** — 2–3 non-obvious rules you will enforce
3. **Audit commitment** — confirmation that you will run A03 audit before delivery
...
- A02_process.md
- A03_audit.md
- A04_anti_patterns.md

Key constraints retained:
- Section 1 and 2 are immutable protected regions
- All external packages must come via C00_set_packages
- No code executes outside Section 99

Audit commitment:
- I will run the A03 audit and will not deliver code with CRITICAL or MAJOR violations.
```

If you cannot produce this confirmation, you are **not permitted to implement code**.
...
| ---------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| 1. Governance    | `A00_start_here.md` → `A01_rules.md` → `A02_process.md` → `A03_audit.md` → `A04_anti_patterns.md`  | How to behave, what’s prohibited, how to audit   |
...
* **Detailed API docs next** — Full signatures, return values, error handling, and rationale
* **Quick lookups last** — Indexes only; they verify task-to-function mapping
...
**Not this (usage-level violation):**

```python
# ❌ WRONG — Bypasses Core utilities
pd.read_csv(path)
```

**Not this (import-level violation):**

```python
# ❌ WRONG — Direct external import (violates Rule I-1)
import pandas as pd
df = pd.read_csv(path)
```
**Not this (import-level violation — standard library):**

```python
# ❌ WRONG — Direct standard library import (violates Rule I-1)
...
* Section numbers: 1, 2, 3–97, 98, 99
...
| -------------------------------------- | ---------------------------------------------------- |
| Python code in this repository         | Full rules (A01–A04 audit required)                  |
...
## 9. Blockers, Failures, and Recovery

### Blocked or Partial Work

If you encounter a blocker mid-task (missing information, architectural conflict, required permission):

* Stop implementation immediately
* Clearly state what is complete and what is blocked
* Identify the specific rule or constraint causing the halt
* Ask for resolution before continuing

Do NOT deliver speculative, placeholder, or guessed implementations.
...
1. Stop immediately
2. Identify the violated rule (file + rule number)
3. Correct the issue
4. Re-run the A03 audit

Repeated violations for the **same task** are treated as governance failures and **must be explicitly flagged to the user for resolution before proceeding with that task**.
